...
//        //
//        stepper2.setupMoveInRevolutions(3.0);
//
//        //
//        // execute the moves
//        //
...
//
void FlexyStepper::setCurrentPositionInMillimeters(float currentPositionInMillimeters)
{
  setCurrentPositionInSteps((long) round(currentPositionInMillimeters * stepsPerMillimeter));
}
...
//
void FlexyStepper::setAccelerationInMillimetersPerSecondPerSecond(float accelerationInMillimetersPerSecondPerSecond)
{
    setAccelerationInStepsPerSecondPerSecond(accelerationInMillimetersPerSecondPerSecond * stepsPerMillimeter);
}
...
//
bool FlexyStepper::moveToHomeInMillimeters(long directionTowardHome, float speedInMillimetersPerSecond, 
...
//
void FlexyStepper::setTargetPositionRelativeInMillimeters(float distanceToMoveInMillimeters)
{
...
//
void FlexyStepper::moveToPositionInMillimeters(float absolutePositionToMoveToInMillimeters)
{
...
//
void FlexyStepper::setTargetPositionInMillimeters(float absolutePositionToMoveToInMillimeters)
{
...
//
void FlexyStepper::setCurrentPositionInRevolutions(float currentPositionInRevolutions)
{
  setCurrentPositionInSteps((long) round(currentPositionInRevolutions * stepsPerRevolution));
}
...
//
void FlexyStepper::setAccelerationInRevolutionsPerSecondPerSecond(float accelerationInRevolutionsPerSecondPerSecond)
{
    setAccelerationInStepsPerSecondPerSecond(accelerationInRevolutionsPerSecondPerSecond * stepsPerRevolution);
}
...
//  Exit:   true returned if successful, else false
//
bool FlexyStepper::moveToHomeInRevolutions(long directionTowardHome, float speedInRevolutionsPerSecond, 
...
//
void FlexyStepper::setTargetPositionRelativeInRevolutions(float distanceToMoveInRevolutions)
{
...
//
void FlexyStepper::moveToPositionInRevolutions(float absolutePositionToMoveToInRevolutions)
{
...
//
void FlexyStepper::setTargetPositionInRevolutions(float absolutePositionToMoveToInRevolutions)
{
 setTargetPositionInSteps((long) round(absolutePositionToMoveToInRevolutions * stepsPerRevolution));
}
...
//  Exit:   true returned if successful, else false
//
bool FlexyStepper::moveToHomeInSteps(long directionTowardHome, float speedInStepsPerSecond, 
...
void FlexyStepper::setTargetPositionToStop()
{
  ...
  //
  decelerationDistance_InSteps = (long) round(5E11 / (acceleration_InStepsPerSecondPerSecond * currentStepPeriod_InUS * currentStepPeriod_InUS));
...
{ 
  ...
  delayMicroseconds(2);        // set to almost nothing because there's so much code between rising & falling edges
...
//
// determine the period for the next step, either speed up a little, slow down a little 
// or go the same speed
//
void FlexyStepper::DeterminePeriodOfNextStep()
{
  ...
  //
  // determine the number of steps needed to go from the current speed down to a velocity of 0
  // Steps = Velocity^2 / (2 * Acceleration)
  //
  ...
  if ((directionOfMotion == 1) && (targetInPositiveDirectionFlag))
  {
    ...
    //
    if ((distanceToTarget_Unsigned < decelerationDistance_InSteps) || (nextStepPeriod_InUS < desiredPeriod_InUSPerStep))
      slowDownFlag = true;
    else 
      speedUpFlag = true;
   }
