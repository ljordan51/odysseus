...
void loop()
{
  ...
  while(notDone){
    ...
    stepperZ.setSpeedInRevolutionsPerSecond(1); // start slow
    stepperZ.setAccelerationInRevolutionsPerSecondPerSecond(1); // random acceleration
    stepperZ.setTargetPositionRelativeInRevolutions(-100); // just run for a while idk
    unsigned long start_time = millis();

    while(!stepperZ.motionComplete()){
      unsigned long cur_time = millis()-start_time;
      stepperZ.setSpeedInRevolutionsPerSecond(1*(1+cur_time/1000));
//      if(cur_time > 1000 && cur_time < 2000){
